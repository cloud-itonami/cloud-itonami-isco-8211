(ns mechassemblycoord.actor
  "MechAssemblyCoordActor — the ISCO-08 8211 mechanical machinery
  assemblers assembly-line scheduling/logistics coordination actor as
  a `langgraph.graph/state-graph` (ADR-2607121000 / CLAUDE.md Actors
  section). One graph run = one assembly-line coordination operation
  ...
  The unconditional invariant: the Mechanical Assembly Line Scheduling
  Coordination Advisor can never directly commit an operating record
  the MechAssemblyCoordGovernor refuses, and can never make this actor
...
(defn build-graph
  "Build a compiled MechAssemblyCoordActor graph. `store` implements
  `mechassemblycoord.store/Store`. `advisor` implements
  `mechassemblycoord.advisor/Advisor` (defaults to `mock-advisor`).

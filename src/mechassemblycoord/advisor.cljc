(ns mechassemblycoord.advisor
  "Mechanical Assembly Line Scheduling Coordination Advisor — proposing
  an assembly-line scheduling/logistics coordination operation (log a
  work record, schedule a crew operation, flag a safety concern,
  coordinate a components/fasteners-stock supply order) from a crew
  roster, line registration and safety-reporting policy. Swappable
  mock/llm; the advisor ONLY proposes — `mechassemblycoord.governor`
  ...
  scope. Modeled closely on cloud-itonami-isco-8122's
...
(defn- rationale-for [op assembler-id line-id hazard-type]
  (case op
    ...
    (str "scheduled crew operation for assembly line task at line " line-id)
...
(def ^:private system-prompt
  "You are a mechanical machinery assemblers assembly-line
   ...
   :assembler-id, :line-id, and any :cost/:hazard-type/:task fields,
   an honest :confidence and a :stake. Never propose an op outside
   this closed list, and never propose to directly finalize an
   assembly-execution decision (e.g. deciding to proceed with a
   specific assembly run), or a line-safety-clearance decision (e.g.
   declaring an assembly line safety cleared), or to override a plant
   safety officer's judgment — those are always out of this actor's
   scope; it coordinates assembly-line scheduling/logistics only and
   never performs assembly work or clears a line for operation itself.
